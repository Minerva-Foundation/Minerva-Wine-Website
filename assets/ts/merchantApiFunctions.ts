...
export async function getMerchantDetailBySlug(slug: String, sanity: any ):  Promise<defTypes.MerchantDetails> {
    const query = groq`*[_type == "merchant" && slug.current == "${slug}"]{_id,country,description,description2,description3,flag,largeFirstImage,largeSecondImage,map,quote,seo,slug,title}`;
